...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, base_model_dir: Optional[str] = None) -> None:
        super().__init__()
        base_model_dir = base_model_dir or "wan_models/Wan2.1-T2V-1.3B"
        ...
        self.text_encoder.load_state_dict(
            torch.load(os.path.join(base_model_dir, "models_t5_umt5-xxl-enc-bf16.pth"),
        ...
        self.tokenizer = HuggingfaceTokenizer(
            name=os.path.join(base_model_dir, "google", "umt5-xxl"), seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self, base_model_dir: Optional[str] = None):
        ...
        self.model = _video_vae(
            pretrained_path=os.path.join(base_model_dir, "Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        model_dir = base_model_dir or f"wan_models/{model_name}"
